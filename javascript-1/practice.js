...
// console.log(colorCopy.push('lavender'))
...
let doubled = myNumbers.map(function(answer){
	return answer *= 2;
});
...
let filtered = myNumbers.filter(function(greater){
	return greater > 100;
})
...
let total = myNumbers.reduce(function(acc, num) {
	return acc += num;
})
...
let forTheLoveOfGeorge = notGeorge.map(function(newGeorges){
	return newGeorges = 'George';
})
...
let enemies = people.filter(function(unfriends){
	return unfriends.friend === false;
})
...
let totallyAwesome = people.reduce(function(accu, num, index, arr){
	console.log(arr.length)
	return accu += num.awesomeLevel;
}, 0)
